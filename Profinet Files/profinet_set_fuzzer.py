...
def generate_random_hex_bytes(byte_count):
    return random.getrandbits(byte_count * 8)


def generate_random_hex_bytes_as_str(byte_count, check=''):
    if not check:
        data = ''.join(['{:02x}'.format(random.randint(1, 255)) for _ in range(byte_count)])
        return data
    else:
        return byte_count * check
...
class DCPSetPacket:
    ...
    def prepare_packet(self):
        ...
        self.payload = self.packet_format.pack(*pack_args).hex()
...
if __name__ == '__main__':
    ...
    for option in options_data.keys():
        with open('profinet_set_fuzzer.log_%02x' % option, 'w') as fh_log, open('profinet_set_fuzzer.err_%02x' % option, 'w') as fh_err:
            for suboption in options_data[option]:
                ...
                for pck_num in range(1, packets_per_suboption + 1):
                    ...
                    try:
                        ...
                        pp = Ether(type=0x8892, src=src_mac, dst=dst_mac) / bytes.fromhex(payload)
                        ans, unans = srp(pp, timeout=cfg_sniff_time)
                        response = 'NO RESPONSE'
                        answer_code = -1
                        if ans:
                            response = hexlify(ans[0][1].load).decode('utf-8')
                            answer_code = int(response[36:38], 16)
                        else:
                            fh_err.write("%s\nresponse: %s\n" % (info_text, str(ans)))
                        fh_log.write("response: %s\n" % response)
                        if answer_code != -1:
                            answer_text = dcp_answers.get(answer_code, 'answer unknown')
